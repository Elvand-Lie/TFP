...
function renderChart(data, input) {
  ...
  if (data.qmdj && qmdjGrid && qmdjMeta && qmdjSection) {
    ...
    function getElementClass(element) {
      if (!element) return '';
      const m = { 'Wood': 'el-wood', 'Fire': 'el-fire', 'Earth': 'el-earth', 'Metal': 'el-metal', 'Water': 'el-water' };
      return m[element] || '';
    }
    ...
    luoShuOrder.forEach(id => {
      ...
      if (isKongWang || isTianMa || isMingGong) {
        badgesHtml = '<div class="qmdj-badges">';
        if (isKongWang) badgesHtml += '<span class="qmdj-badge qmdj-badge-void" title="Kong Wang (Void · 空亡)">空</span>';
        if (isTianMa) badgesHtml += '<span class="qmdj-badge qmdj-badge-horse" title="Tian Ma (Sky Horse · 驛馬)">馬</span>';
        if (isMingGong) badgesHtml += '<span class="qmdj-badge qmdj-badge-ming" title="Ming Gong (Destiny Palace · 命宮)">命</span>';
        badgesHtml += '</div>';
      }
